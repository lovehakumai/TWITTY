...
def get_posts(user_id):
    ...
    post_list = {}
    temp_list = select(
        'SELECT follow_to FROM Relations WHERE followed_by=? GROUP BY 1',
        user_id
    )
    if len(temp_list)==0:
        return None
        ...
        post_dict = {}
        # SQLクエリを正しくフォーマットする({placeholders}をそのままSQL分に入れても展開されないので.formatを利用した)
        query = 'SELECT * FROM Post_contents INNER JOIN (SELECT user_id, nickname, thumbnail_url FROM Users) as A ON Post_contents.user_id=A.user_id WHERE A.user_id IN ({});'.format(placeholders)

        temp_post_dict = select(query, *user_ids)
